package com.example.restexamplei.repository;

import com.example.restexamplei.exception.LiveStreamNotFoundException;
...
public class LiveStreamRepository {
    ...
    public LiveStreamRepository() {
        streams.add(new LiveStream(
                ...
                "Spring Boot is very convenient to use when building REST APIs; it allows you to start with minimal configurations." +
                         "But there’s always room for trouble to creep in. " +
                         "Join us for the next IntelliJ IDEA Live Stream to learn how best to avoid this trouble in developing your project. " +
                         "During the February show, Dan Vega will show us how to make sure we’re following good practices when working with Spring Initializr.",
    ...
    public LiveStream findById(String id) throws LiveStreamNotFoundException {
        return streams.stream().filter(stream -> stream.getId().equals(id)).findFirst().orElseThrow(LiveStreamNotFoundException::new);
    }
    ...
    public void update(LiveStream stream, String id) {
        LiveStream existing = streams.stream().filter(s -> s.getId().equals(id))
                .findFirst()
